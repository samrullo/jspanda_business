...
class ShipmentWeightController:
    ...
    def prepare_view_from_shipment_weight_records(self,records:List[ShipmentWeight],title:str="Shipment weights"):
        total_weight = sum([record.weight for record in records])
        total_amount = sum([record.amount for record in records])
        total_amount_usd = sum([record.amount_usd for record in records])        
        order_col_no=0        
    ...
    def show_pending_shipment_weights(self):
        ...
        return self.prepare_view_from_shipment_weight_records(records,"Pending Shipment Weights")

    def show_all_shipment_weights(self):
        records = ShipmentWeight.query.order_by(ShipmentWeight.date.desc()).all()
        return self.prepare_view_from_shipment_weight_records(records,"All Shipment Weights")

    def show_shipment_weights_by_date(self):
        df = pd.read_sql(ShipmentWeight.query.statement, ShipmentWeight.query.session.bind)
        shipments_df = df.groupby('date').sum()[['weight', 'amount','amount_usd']]
        shipments_df["date"]=shipments_df.index
        shipments_df.sort_index(ascending=False, inplace=True)
        total_weight = shipments_df["weight"].sum()
        total_amount = shipments_df["amount"].sum()
        total_amount_usd = shipments_df["amount_usd"].sum()
        order_col_no=0        
        ...
        records=[DotDict({col:row[col] for col in shipments_df.columns}) for i,row in shipments_df.iterrows()]
        col_names=[DotDict({"description":"Date","db_name":"date","type":"date"}),        
        DotDict({"description":"Weight","db_name":"weight","type":"numeric"}),
        DotDict({"description":"Amount USD","db_name":"amount_usd","type":"numeric"}),
        DotDict({"description":"Amount JPY","db_name":"amount","type":"numeric"}),
        ]        
        return render_template("generic_table_with_extra_funcs_two.html",
        title="Shipment Weights by date",
        summary_records=summary_records,
        order_col_no=order_col_no,
        col_names=col_names,
        records=records,
        ...
        add_func_name=None,
        edit_func_name=None,
        remove_func_name=None,
        extra_funcs=[DotDict({"name":'admin_bp.show_adate_shipment_weight',
        "var_name":"adate",
        "db_col_name":"date",
        "processing_func":to_yyyymmdd,
        "link_name":"View"})])
    ...
    def show_adate_shipment_weight(self,adate:str):
        as_of=datetime.datetime.strptime(adate,"%Y%m%d")
        records=ShipmentWeight.query.filter(ShipmentWeight.date==as_of).order_by(ShipmentWeight.date.desc()).all()
        return self.prepare_view_from_shipment_weight_records(records,f"Shipment Weights as of {adate}")

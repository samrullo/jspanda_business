...
def save_receipt_image_and_update_spending_db_record(form:FlaskForm,spending,images:UploadSet,db:SQLAlchemy):
    """
    Save receipt image into receipts folder
    """
    receipt_filename=f"{spending.name.lower().replace(' ','_')}_{spending.spending_category.name.lower()}_{spending.payment_method.name.lower().replace('/','_')}_{datetime.datetime.strftime(spending.spent_at,'%Y%m%d_%H%M%S')}."
